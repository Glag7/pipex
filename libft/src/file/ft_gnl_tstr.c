...
/*   Updated: 2024/01/12 22:47:57 by glaguyon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	*ft_free1024(t_list **readed)
{
	...
	while (i < 1024)
	{
		ft_lstclear(readed + i, &ft_tstrdel);
		readed[i] = 0;
		i++;
	}
	return (NULL);
...
static char	*ft_freegnl(t_list **readed, int fd)
{
	if (fd < -1024)
		...
		ft_lstclear(readed - fd - 1, &ft_tstrdel);
	return (NULL);
}
...
t_str	ft_gnl_tstr(int fd, size_t bsize)
{
	...
	line = ft_lsttstr_to_tstr(readed + fd, len, &ft_tstrdel, &ft_iseol);
	if (line.s == NULL)
		ft_free1024(readed);
	return (line);
}
